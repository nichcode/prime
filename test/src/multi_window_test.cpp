...
static void on_window_close(const Window* window)
{
	str msg = std::format("{0} - closed", window->get_title());
	PINFO(msg);
}
...
static void on_window_focused(const Window* window, b8 focused)
{
	str msg;
	if (focused) {
		msg = std::format("{0} - focused", window->get_title());
	}
	else {
		msg = std::format("{0} - not focused", window->get_title());
	}
	PINFO(msg);
}

b8 multi_window_test()
{
	...
	props.title = "Multi Window Test Window 1";
	props.width = 800;
	props.x_pos = 200;
	props.y_pos = 200;
	s_window.init(props);
	...
	props2.title = "Multi Window Test Window 2";
	s_window2.init(props2);

	set_window_close_callback(on_window_close);
